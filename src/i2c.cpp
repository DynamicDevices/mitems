...
void i2c_init(uint8_t scl_pin, uint8_t sda_pin)
{
    ...
    gpio_set_pull_mode( (gpio_num_t) g_scl_pin,GPIO_PULLUP_ONLY);
    gpio_set_pull_mode( (gpio_num_t)g_sda_pin,GPIO_PULLUP_ONLY);

    gpio_set_direction( (gpio_num_t)g_scl_pin,GPIO_MODE_INPUT_OUTPUT);
    gpio_set_direction( (gpio_num_t)g_sda_pin,GPIO_MODE_INPUT_OUTPUT);

    // I2C bus idle state.
    gpio_set_level((gpio_num_t)g_scl_pin,1);
    gpio_set_level((gpio_num_t)g_sda_pin,1);
}
...
//    gpio_write(g_scl_pin, 1);
	gpio_set_level((gpio_num_t)g_scl_pin,1);
    return gpio_get_level((gpio_num_t)g_scl_pin); // Clock high, valid ACK
}
...
static inline bool read_sda(void)
{
    ...
    i2c_delay();
    return gpio_get_level((gpio_num_t)g_sda_pin); // Clock high, valid ACK
}

// Actively drive SCL signal low
static inline void clear_scl(void)
{
	gpio_set_level((gpio_num_t)g_scl_pin, 0);
}

// Actively drive SDA signal low
static inline void clear_sda(void)
{
	gpio_set_level((gpio_num_t)g_sda_pin, 0);
}
